...
function Navbar() {
  ...
  const showOweData = () => {
    let sumOfAmt = 0;
    let arrayOfAmtToOweOthers = JSON.parse(
      localStorage.getItem("arrayOfExpData")
    );
  ...
  const showReturnsData = () => {
    ...
    arrayOfAmtToOweOthers.map((elem) => {
      return (sumOfAmt =
        sumOfAmt +
        Number(Number(elem.amt) - Number(elem.amt) / Number(elem.noOfPeople)));
    });
